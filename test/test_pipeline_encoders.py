...
class PipelineEncoderTest(unittest.TestCase):
    ...
    def test_standard_detection_and_segment_encoders(self) -> None:
        ...
        segment = self.make_sample(
            (Polygon(label='dial', points=((10, 10), (90, 10), (50, 90))),)
        )
        self.assertEqual(
            ('0 0.500000 0.500000 0.500000 0.500000',),
            EncodeDetection().transform(detection, context).lines,
        )
        self.assertEqual(
    ...
    def test_crop_detection_uses_crop_sample_info(self) -> None:
        ...
        sample = self.make_sample(
            (Bbox(label='dial', x1=2.5, y1=2.5, x2=7.5, y2=7.5),),
            size=(10, 10),
        )
        crop = CropOutput(
            sample=sample,
        ...
        self.assertEqual(
            ('0 0.500000 0.500000 0.500000 0.500000',),
            EncodeCropDetection().transform(crop, context).lines,
        )
    ...
    def test_point_segment_reproduces_legacy_triangle(self) -> None:
        ...
        crop = CropOutput(
            sample=sample,
            parent=Bbox(label='cc', x1=0, y1=0, x2=40, y2=40),
        )
        output = EncodePointSegment().transform(crop, context)
        self.assertEqual(
            ('0 0.182918 0.283541 0.317082 0.216459 0.500000 0.750000',),
            output.lines,
        )
    ...
    def test_knob_segment_clamps_normalized_coordinates(self) -> None:
        ...
        output = EncodeKnobSegment().transform(
            CropOutput(sample=sample, parent=parent),
            context,
        )
        self.assertEqual(
            ('0 0.000000 0.100000 1.000000 0.100000 0.500000 1.000000',),
            output.lines,
        )
    ...
    def test_pose_encoder_uses_catalog_order(self) -> None:
        ...
        item = MatchOutput(
            sample=sample,
            matches=(AnnotationMatch(parent=parent, children=(point,)),),
        )
        output = EncodePose().transform(item, context)
        self.assertEqual(
            ('0 0.500000 0.500000 0.800000 0.800000 0.200000 0.300000 2',),
            output.lines,
        )
