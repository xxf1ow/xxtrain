...
第一阶段的数据、pipeline、training、打包和安装式 CLI 已经完成，迁移后的格式互转和分类无标签目录 review 也已落地。进入第二阶段实现前，先补齐目录源目标标签子集转换与统计、源数据变化检测两项近端能力，再实现最小离线半监督核心，最后增加薄平台。
...
| 目录标注目标子集转换与统计 | 待实现 | 标准任务继续由 `src/labels.txt` 定义目标类别；转换时统计全部来源标签、过滤非目标标签，并校验最终类别覆盖 |
...
1. 实现目录标注的目标子集转换与标签统计，保持显式类别所有权并校验最终类别覆盖；
...
“发现标注文件”不等于“发现类别目录”。普通目录源不扫描所有 XML/JSON 汇总 `LabelCatalog`；标准任务始终以 `src/labels.txt` 为类别名称与顺序的真值来源。M1 将在同一次转换中统计全部来源标签、过滤并报告非目标标签，再按完整处理链之后的最终类别验证目标覆盖；它不会自动扩展类别目录。COCO 可从自身 category schema 提供目录，特殊 Recipe 则显式携带固定目录和转换规则。
