...
- `xxtrain.platform.training_service` 在标注写互斥内计算当前输入指纹，返回该用户和目标已有的输入运行，或在资格与缓存准备完成后创建服务端运行身份并持久关联 ClearML 任务；终态运行只供观察，未知远端状态继续锁定编辑。提供训练配置时，平台组合根在独立元数据目录构造运行账本和 ClearML 适配器，并把服务的实时编辑保护绑定到 `AnnotationService`；省略训练配置时保留仅标注启动方式；
