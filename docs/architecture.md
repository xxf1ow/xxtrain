...
- `xxtrain.platform.training_service` 在标注写互斥内校验资格、准备缓存并持久关联 ClearML 任务；相同请求恢复既有任务，未知远端状态继续锁定编辑，历史重试只使用原运行保留的缓存。`AnnotationService` 的上传、开始标注和同步入口调用组合根提供的编辑保护，但缓存准备和其他已合格目标的提交不受该保护阻断；
