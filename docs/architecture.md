...
公共组件的任务规则统一与 Point 并行依赖调整见[任务定义驱动提案](agent-notes/proposed/architecture/2026-09-19-task-definition-driven-platform.md)。任务规则、存储校验、工作区 frame 投影、同步、失效、当前输入指纹、平台标注协调、CVAT 组合和训练缓存已按定义派生；训练保护、HTTP 动态路由、页面和 worker 仍保留后续迁移范围。
