...
xxtrain 是围绕 Ultralytics YOLO 的已安装 Python 包和训练工具。当前系统读取外部标注，把样本转换为任务数据集，生成模型配置，准备预训练权重，执行训练与预测检查，并导出 ONNX 模型。`xxtrain.workspace_data` 已实现预置平铺图片和同 stem LabelMe 文件的无损检测标注存储边界；仓库尚未实现完整用户平台、SQLite 元数据、CVAT/ClearML 连接器、业务工作区流程或训练快照。
...
内部自助训练平台仍处于提案阶段；当前只实现共享数据类型和无损检测标注存储边界，页面、业务流程及 CVAT/ClearML 连接器尚未实现。平台边界、路线和验收标准由 [内部自助训练平台 Agent Note](agent-notes/proposed/feature/2026-07-30-self-service-training-platform.md) 所有，未实现内容不属于当前架构。
