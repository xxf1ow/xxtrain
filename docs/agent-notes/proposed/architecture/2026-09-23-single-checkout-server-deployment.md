...
新站点的 ClearML 平台服务身份由 `install` 随机生成一次 API access key 与 secret key，保存到私有的 `.deployment/platform.env`，已有完整凭据保持原样。配置生成步骤从该文件派生 `.deployment/clearml/config/secure.conf` 的直接身份 `secure.credentials.user`，包含固定镜像初始化器读取的 `role: user`、`user_key`、`user_secret` 和 `display_name`；该角色值与固定镜像默认 `tests` 身份一致。由于固定镜像按文件名将此文件索引为 `secure` 配置节点，文件正文从 `credentials` 开始，不再包裹额外的 `secure` 节点，才能使初始化器读取 `secure.credentials.user`。派生文件不是第二份可手工编辑的凭据来源，ClearML API 容器启动前缺失或过期时可从权威文件重建。固定镜像的 `BasicConfig` 在默认配置后读取已挂载的 `/opt/clearml/config/*.conf`，Mongo 初始化据此创建服务用户并将配置中的 key/secret 作为认证凭据。平台进程直接使用 `platform.env` 的同一对值；引导通过 HTTP Basic `GET /auth.login` 验证响应中有 `data.token` 且 `meta.result_code` 为 200，不以匿名健康检查代替，不调用需要先登录的 `auth.create_credentials`，也不要求用户进入 ClearML Web 签发密钥。已有凭据格式不完整或与已运行的服务不一致时显式报错，不悄悄轮换。该请求遵循已安装 ClearML SDK 的认证路径；首次现场部署仍须验证固定镜像中的配置身份可通过真实认证请求。
